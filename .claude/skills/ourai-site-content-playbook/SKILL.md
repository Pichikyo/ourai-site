...
- `category`: **`PRESS / TOPICS / WORKS` の3種（英語表記）に固定**（2026/07/12改定・語彙外はsyncが警告）。使い分け＝WORKS:制作・公開・刊行など成果系（実績連動の1行は常にこれ）／TOPICS:出展・登壇予告・認定・サイト更新など会社の動き／PRESS:PR TIMES等の正式配信時。実績の細分類はしない（分類はworks.htmlのタグフィルタの役割）
