...
- `category`: **`PRESS / TOPICS / WORKS / MEDIA` の4種（英語表記）に固定**（2026/07/12確定・語彙外はsyncが警告）。使い分け＝WORKS:制作・公開・刊行など成果系（実績連動の1行は常にこれ）／TOPICS:出展・登壇予告・認定・サイト更新など会社の動き／PRESS:PR TIMES等の正式配信時／MEDIA:外部メディアへの掲載・取材・寄稿。実績の細分類はしない（分類はworks.htmlのタグフィルタの役割）。これ以上タグを増やさない
...
**連動規約（2026/07/12確立）**: 実績台帳（ourai-works-ledger.json）に**公開案件を追加したら、同じ作業ターンでニュース台帳にも1行追加**してからsyncする。**newsのtextはworksのタイトルと全く同じでよい**（言い換え不要・オーナー確認済み）。日付はnews側で `YYYY.MM` に単月化し、linkは該当の特集ページまたは works.html へ。ニュース固有ネタ（予告・プレスリリース・メディア掲載・認定）は実績と無関係に随時追加してよい。
...
## ニュース運用の追加決定（2026-07-12 智美）

- **リンクは登録時に手動指定**（実在ページ限定：特集ページ・プレス記事・外部記事）。works.html等への自動リンク付与は廃止。行き先がなければリンクなし
- **ニュースは2階建て**：①通常の告知＝一覧に1行のみ ②プレス級（PR TIMES配信と同等の文章量があるもの）＝個別記事ページを作成し、台帳のlinkに指定。個別ページの本文はPR TIMESの丸写しではなく自サイト版を正本として調整（SEO対策）
- **個別記事ページの仕様**：タイトル／日付／カテゴリ／本文／「← ニュース一覧に戻る」のみ（前後記事リンクは付けない）。テンプレは最初のプレス級記事を書くときに合わせて作る（先行して作らない）
- **ページネーションは当面なし**。一覧は年別見出しで区切る。100件超えたら「直近3年＋アーカイブ」方式を再検討
- 一覧の各項目には `id="N0xx"` のアンカーがあり、`news.html#N036` の形で個別参照できる（個別ページの代替）
