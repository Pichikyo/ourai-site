...
**連動規約（2026/07/12確立）**: 実績台帳（ourai-works-ledger.json）に**公開案件を追加したら、同じ作業ターンでニュース台帳にも1行追加**してからsyncする（文言は実績のコピペではなくニュースの言い回し。例:「レーザー技術を学ぶ「Kyocera Laser World」を公開しました」→ news側は `「Kyocera Laser World」公開` 型に圧縮、linkで実績・特集へ誘導）。ニュース固有ネタ（予告・プレスリリース・メディア掲載・認定）は実績と無関係に随時追加してよい。
