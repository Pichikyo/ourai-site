...
- `category`: **`プレスリリース/トピックス/実績` の3種に固定**（2026/07/12改定・語彙外はsyncが警告）。使い分け＝実績:制作・公開・刊行など成果系（実績連動の1行は常にこれ）／トピックス:出展・登壇予告・認定・サイト更新など会社の動き／プレスリリース:PR TIMES等の正式配信時。実績の細分類はしない（分類はworks.htmlのタグフィルタの役割）
